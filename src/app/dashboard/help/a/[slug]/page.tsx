    ...
    (async () => {
      try {
        const res = await fetch(
          `/api/help/articles/${encodeURIComponent(slug)}?locale=${HELP_DEFAULT_LOCALE}`
        );
        const j = await res.json();
        if (cancelled) return;
        if (!res.ok) {
          setErr(j.error || "Niet gevonden");
          return;
        }
        setTitle(j.article.title);
        setBody(j.article.body);
      } catch {
        if (!cancelled) setErr("Laden mislukt");
      }
    })();
    return () => {
      cancelled = true;
