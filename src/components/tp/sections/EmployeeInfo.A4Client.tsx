  ...
  useLayoutEffect(() => {
    ...
    const measureAndPaginate = () => {
      ...
      imagePromise
        .then(() => {
          ...
          requestAnimationFrame(() => {
            requestAnimationFrame(() => {
              ...
              const heights = measurables.map((_, i) => {
                const el = blockRefs.current[i];
                const height = el?.offsetHeight ?? 0;
                if (height === 0 && el) {
                  console.warn(`⚠️ EmployeeInfoA4Client: Block ${i} has height 0`, {
                    element: el,
              ...
              if (heights.every(h => h === 0) && measurables.length > 0) {
                console.warn('⚠️ EmployeeInfoA4Client: All heights are 0, retrying...');
                setTimeout(() => {
                  const retryHeights = measurables.map((_, i) => blockRefs.current[i]?.offsetHeight ?? 0);
                  if (retryHeights.some(h => h > 0)) {
                    calculatePages(retryHeights, firstH, restH);
                  } else {
                    console.error('❌ EmployeeInfoA4Client: Still all heights 0 after retry');
                    setPages([]);
                  }
                }, 200);
                return;
        ...
        .catch((err) => {
          console.error('❌ EmployeeInfoA4Client: Measurement failed', err);
