...
import { sanitizeIntakeSectie3Content } from './build-fields';
import { DEFAULT_INTAKE_SECTIE3_MODEL, INTAKE_DOC_VARIANTS } from './constants';
import {
  INTAKE_SECTIE3_CONTENT_PROMPT,
  buildIntakeSectie3ContextMessage,
} from './prompt';
import {
...
function getIntakeSectie3Model(): string {
  return process.env.OPENAI_INTAKE_SECTIE3_MODEL?.trim() || DEFAULT_INTAKE_SECTIE3_MODEL;
}

function getReasoningEffort(): 'low' | 'medium' | 'high' | undefined {
  const raw = process.env.OPENAI_INTAKE_SECTIE3_REASONING?.trim().toLowerCase();
...
async function uploadIntakeDocs(
  ...
  for (const doc of docs.filter((d) => isIntakeDoc(d.type))) {
    ...
    if (totalBytes + buffer.length > MAX_UPLOAD_BYTES) {
      console.warn('⚠️ Intake sectie 3: skipping document (size limit)', path);
      continue;
    }
...
export async function generateIntakeSectie3Content(
  openai: OpenAI,
  supabase: SupabaseClient,
  docs: EmployeeDoc[]
): Promise<IntakeSectie3Content> {
  ...
  try {
    const contextMessage = buildIntakeSectie3ContextMessage();
    ...
    const response = await openai.responses.create({
      model: getIntakeSectie3Model(),
      instructions: INTAKE_SECTIE3_CONTENT_PROMPT,
      input: [{ role: 'user', content: userContent }],
      ...(reasoningEffort ? { reasoning: { effort: reasoningEffort } } : {}),
      text: {
        format: {
          type: 'json_schema',
          name: 'intake_sectie3_content',
          strict: true,
          schema: INTAKE_SECTIE3_JSON_SCHEMA as Record<string, unknown>,
        },
      },
    });
    ...
    return sanitizeIntakeSectie3Content(parseIntakeSectie3Content(parsed));
  } finally {
